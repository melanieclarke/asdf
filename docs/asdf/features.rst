...
..
   An example for using the csv data would be helpful here.
...
see more information about the the attribute, similar to the way that
...
..
   I find this example a little hard to follow, since I'm not familiar with what
   is meant by archive_catalog, and "thing1/2" is very generic.  A minimal real
   example would be more helpful, since it would allow me to test it while following
   along - is there a built-in schema that might have useful descriptions to
   search?
